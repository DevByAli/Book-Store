export function modifyCloudinaryUrl(url: string, width: number, height: number): string {
    ...
    const modifiedUrl = url.slice(0, uploadIndex) + sizeParams + '/' + url.slice(uploadIndex);
